...
function init_UserItems(namespace)
{
    ...
    /************************************************************** {
     * Monitor option group change events and on form submission
     * if we can handle the change client-side, do it.
     *
     * :NOTE: This doesn't currently work because the backend
     *        only generates the options that were requested.
     *
    // Bind to any submit events from within this list
    $list.bind('change.optionGroup', function(e, data) {
        var changed = $list.data('changed');
        if (changed === undefined)
            changed = {};

        if (data !== undefined)
            changed['optionGroup'] = data;
        else
        {
            var $target = $(e.target);
    ...
    $list.submit(function(e) {
        ...
        if ( (changed                !== undefined) &&
             (changed['optionGroup'] !== undefined) &&
             (changed['other']       !== true) )
        {
            // We've ONLY changed option group information.  Apply it directly.
            // and stop this event.
            e.preventDefault();
            e.stopPropagation();

            var prefix  = 'ul.'+ namespace +' ';

            // Show all items that should be visible...
            var selector    = prefix
                            + changed['optionGroup'].selected
                                                        .join(',')
                                                        .replace(/,/g,
                                                                 ','+ prefix);
            $list.find(selector).show();

            // And hide those that should now.
            selector = prefix
                     + changed['optionGroup'].deSelected
                                                .join(',')
                                                .replace(/,/g, ','+ prefix);
            $list.find(selector).hide();

            // Re-enable the display-options and close the form
            var $displayOptions = $(e.target).parent('.displayOptions');
            if ($displayOptions.length > 0)
            {
                $displayOptions.dropdownForm('enable')
                               .dropdownForm('close');
            }
