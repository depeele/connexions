...
abstract class Model_Mapper_Base extends Connexions_Model_Mapper_DbTable
{
    ...
    public function fetchRelated( array $params = array())
    {
        ...
        if (! empty($specials))
        {
             ...
             * table(s).
             */
            $secSelect = $this->_addJoins($select, $secSelect,
                                          $specials, 'uti');
        }
     ...
     *
     *  @return A new secondary Zend_Db_Select with additional sub-selects.
     */
                                                ...
                                                $mainAs)
    {
        ...
        foreach ($specials as $table => $info)
        {
            ...
            foreach ($info['fields'] as $field => $bool)
            {
                ...
                array_push($extraMainFields, $mainAs .'.'. $field);
            }
            ...
            // Join the select and sub-select
            $secSelect->join(array($info['as'] => $subSelect),
                             implode(' AND ', $joinCond),
                             null);

            // Include the fields of interest in the columns returned.
            $secSelect->columns($extraFields);
        }

        $select->columns($extraMainFields);
